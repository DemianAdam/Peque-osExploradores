...
export function GroupDetailModal({ group, onClose, initialEditing = false }: GroupDetailModalProps) {
  ...
  const assignedTeacherIds = new Set(formData.selectedTeachers.map((t: { _id: string }) => t._id));
  const availableTeachers = allTeachers?.filter((t: { _id: string; name: string }) => !assignedTeacherIds.has(t._id));

  const handleAddTeacher = (teacherId: string) => {
    const teacher = allTeachers?.find((t: { _id: string; name: string }) => t._id === teacherId);
    if (teacher) {
      setFormData(prev => ({
        ...prev,
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-gray-50 border border-gray-500 rounded-xl p-4">
            ...
            <div className="flex flex-wrap gap-2">
              {group.children.map((c: { _id: string; name: string }) => (
          ...
          <div className={`border rounded-xl p-4 transition-colors ${isEditing ? "bg-emerald-50 border-emerald-200" : "bg-gray-50 border border-gray-500 rounded-xl p-4"}`}>
            ...
            <div className="flex flex-wrap gap-2">
              {formData.selectedTeachers.map((t: { _id: string; name: string }) => (
            ...
            {isEditing && (
              ...
              >
                ...
                {availableTeachers?.map((t: { _id: string; name: string }) => (
